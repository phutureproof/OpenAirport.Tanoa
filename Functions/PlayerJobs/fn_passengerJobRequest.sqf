...
// can we also do this for any other player in the vehicle?
_crew = crew (vehicle _player);
{
    if (isPlayer _x && _x != _player) then {
        [_x, _tip] call OA_fnc_updatePlayerFunds;
        _atcMessage = format [
            "%1 also earned a tip of %2 because they helped %3 finish the mission",
            name _x,
            [_tip] call OA_fnc_formatIntAsCurrency,
            name _player
        ];
